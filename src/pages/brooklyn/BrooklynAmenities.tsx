...
const communityAmenities = [
  { icon: Car, title: "Parking", description: "Street Parking Available" },
    ...
    <Layout>
      ...
      <section className="relative py-32 bg-primary overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <span className="inline-block text-greenland-gold text-sm font-semibold tracking-widest uppercase mb-4">Live Your Best Life</span>
          <h1 className="text-4xl md:text-6xl font-display font-bold text-primary-foreground mb-6">Amenities & Features</h1>
          <p className="text-lg md:text-xl text-primary-foreground/80 max-w-3xl mx-auto leading-relaxed">
            Discover the thoughtful amenities designed to enhance your everyday living at Greenland Brooklyn. Every detail crafted for your comfort.
          </p>
        </div>
      </section>
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto">Explore the beautiful shared spaces designed for relaxation, fitness, and connection</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ourSpaces.map((space, index) => (
              <div key={index} className="group relative h-80 rounded-2xl overflow-hidden shadow-lg">
                <img src={space.image} alt={space.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <div className="flex items-center gap-3 mb-2">
                    <space.icon className="w-5 h-5 text-greenland-gold" />
                    <span className="text-greenland-gold text-sm font-semibold uppercase tracking-wide">{space.label}</span>
                  </div>
                  <h3 className="text-white font-display text-2xl font-bold">{space.title}</h3>
      ...
      <section className="py-20 bg-greenland-cream">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <span className="inline-block text-primary text-sm font-semibold tracking-widest uppercase mb-4">Community Living</span>
            <h2 className="text-3xl md:text-5xl font-display font-bold mb-4">Community Amenities</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">Experience the best of Brooklyn living with our thoughtfully curated amenities</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {communityAmenities.map((amenity, index) => (
              <div key={index} className="group bg-card p-8 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-transparent hover:border-primary/20">
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <span className="inline-block text-primary text-sm font-semibold tracking-widest uppercase mb-4">Your Home</span>
              <h2 className="text-3xl md:text-5xl font-display font-bold mb-6">Apartment Features</h2>
              <p className="text-muted-foreground text-lg leading-relaxed mb-8">
                Every apartment at Greenland Brooklyn is designed with your comfort in mind. From modern finishes to practical conveniences, we've thought of everything.
              </p>
              <div className="grid sm:grid-cols-2 gap-4">
                {apartmentFeatures.map((feature, index) => (
                  <div key={index} className="flex items-start gap-4 p-4 bg-greenland-cream rounded-xl">
      ...
      <section className="py-20 bg-primary">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-display font-bold text-primary-foreground mb-6">Ready to Experience Greenland Brooklyn?</h2>
          <p className="text-primary-foreground/80 text-lg max-w-2xl mx-auto mb-8">Schedule a tour today and discover your new home in Brooklyn's most welcoming community.</p>
          <a href="/locations/brooklyn/contact" className="inline-block bg-greenland-gold hover:bg-greenland-gold-light text-foreground font-semibold px-10 py-4 rounded-lg transition-colors shadow-lg">Contact Us Today</a>
        </div>
      </section>
    </Layout>
  );
...
export default BrooklynAmenities;
