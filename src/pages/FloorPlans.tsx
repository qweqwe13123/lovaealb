...
const floorPlans = [
  ...
  },
  {
    ...
    price: "$1,175",
    image: oneBrImage,
    features: ["Modern kitchen", "In-unit washer/dryer", "City views"],
    hasCarousel: true,
    mediaType: "oneBr",
  },
  {
    ...
    price: "$1,355",
    image: twoBrImage,
    features: ["Modern kitchen", "In-unit washer/dryer", "Extra storage"],
    hasCarousel: true,
    mediaType: "twoBr",
  },
  {
...
const FloorPlans = () => {
  ...
  const nextStudioSlide = () => {
    setStudioIndex((prev) => (prev + 1) % studioMedia.length);
  };

  const prevStudioSlide = () => {
    setStudioIndex((prev) => (prev - 1 + studioMedia.length) % studioMedia.length);
  };

  const nextThreeBrSlide = () => {
    setThreeBrIndex((prev) => (prev + 1) % threeBrMedia.length);
  };

  const prevThreeBrSlide = () => {
    setThreeBrIndex((prev) => (prev - 1 + threeBrMedia.length) % threeBrMedia.length);
  };

  const nextOneBrSlide = () => {
    setOneBrIndex((prev) => (prev + 1) % oneBrMedia.length);
  };

  const prevOneBrSlide = () => {
    setOneBrIndex((prev) => (prev - 1 + oneBrMedia.length) % oneBrMedia.length);
  };

   const nextTwoBrSlide = () => {
     setTwoBrIndex((prev) => (prev + 1) % twoBrMedia.length);
   };
 
   const prevTwoBrSlide = () => {
     setTwoBrIndex((prev) => (prev - 1 + twoBrMedia.length) % twoBrMedia.length);
   };
 
  const getMediaArray = (mediaType: string | null) => {
    if (mediaType === "studio") return { media: studioMedia, index: studioIndex, setIndex: setStudioIndex, next: nextStudioSlide, prev: prevStudioSlide };
    if (mediaType === "threeBr") return { media: threeBrMedia, index: threeBrIndex, setIndex: setThreeBrIndex, next: nextThreeBrSlide, prev: prevThreeBrSlide };
    if (mediaType === "oneBr") return { media: oneBrMedia, index: oneBrIndex, setIndex: setOneBrIndex, next: nextOneBrSlide, prev: prevOneBrSlide };
     if (mediaType === "twoBr") return { media: twoBrMedia, index: twoBrIndex, setIndex: setTwoBrIndex, next: nextTwoBrSlide, prev: prevTwoBrSlide };
    return null;
  };
      ...
      <section className="relative py-24 bg-primary">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-lg text-primary-foreground/80 max-w-2xl mx-auto">
            Find your perfect home at Greenland. Choose from our thoughtfully designed studios, one-bedroom, and two-bedroom apartments.
          </p>
        </div>
      </section>
                  ...
                  {plan.hasCarousel && mediaData ? (
                    <div className="relative h-56 cursor-pointer" onClick={() => openLightbox(mediaData.media, mediaData.index)}>
                      {mediaData.media[mediaData.index].type === "video" ? (
                        <video
                          src={mediaData.media[mediaData.index].src}
                        ...
                        <img
                          src={mediaData.media[mediaData.index].src}
                          alt={`${plan.name} ${mediaData.index + 1}`}
                          className="w-full h-full object-cover"
                        />
                      )}
                      <button
                        onClick={(e) => { e.stopPropagation(); mediaData.prev(); }}
                        className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full transition-colors"
                      >
                        <ChevronLeft className="w-5 h-5" />
                      </button>
                      <button
                        onClick={(e) => { e.stopPropagation(); mediaData.next(); }}
                      ...
                      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
                        {mediaData.media.map((_, idx) => (
                          <button
                            key={idx}
                            onClick={(e) => { e.stopPropagation(); mediaData.setIndex(idx); }}
                            className={`w-2 h-2 rounded-full transition-colors ${
                              idx === mediaData.index ? "bg-white" : "bg-white/50"
                            }`}
                          />
                        ))}
                      </div>
